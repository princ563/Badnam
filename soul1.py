            ...
            if stored_key == key:
                ...
                for user_data in users.values():
                    if user_data.get('redeemed_key') == key:
                        return "This key has already been redeemed. Buy a new key from @Sus_op66."

                if not has_access(user_id):
                    users[str(user_id)] = {'redeemed_key': key, 'has_access': True}
                    save_users(users)

                    bot.send_message(user_id, "Buy a New code From @Sus_op66")
    ...
    try:
        if not has_access(user_id):
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Contact the owner for assistance: @Sus_op66.*", parse_mode='Markdown')
            return
